...
$(function () {
  ...
  if ($grade_detail_graph.length > 0) {
    ...
    %if show_grade_breakdown:
      ...
      $grade_detail_graph.append('<div style="position:absolute;left:' + (o.left - 12) + 'px;top:' + (o.top - 20) + 'px">${"{totalscore:.0%}".format(totalscore=totalScore)}</div>${"{totalscore:.0%}".format(totalscore=totalScore)}');
    %endif
  }

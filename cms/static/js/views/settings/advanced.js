...
var AdvancedView = ValidatingView.extend({
    ...
    attachJSONEditor : function (textarea) {
        ...
        //console.log("attachJSONEditor textarea : "+textarea);
        if ( $(textarea).siblings().hasClass('CodeMirror')) {
            //console.log("attachJSONEditor return ");
            return;
        }

        var self = this;
        var oldValue = $(textarea).val();
        //console.log("attachJSONEditor oldValue : "+oldValue);
        ...
        cm.on('change', function(instance, changeobj) {
                //console.log("attachJSONEditor change : "+instance);
                instance.save();
                // this event's being called even when there's no change :-(
                if (instance.getValue() !== oldValue) {
                    //console.log("attachJSONEditor instance.getValue() : " + instance.getValue() + " : " + oldValue);
                    var message = gettext("Your changes will not take effect until you save your progress. Take care with key and value formatting, as validation is not implemented.");
                    self.showNotificationBar(message,
                                             _.bind(self.saveView, self),
                                             _.bind(self.revertView, self));
        ...
        cm.on('focus', function(mirror) {
                //console.log("attachJSONEditor focus : " + mirror);
                $(textarea).parent().children('label').addClass("is-focused");
            });
        cm.on('blur', function (mirror) {
                //console.log("attachJSONEditor blur : " + mirror);
                ...
                //console.log("attachJSONEditor stringValue:"+stringValue);
                ...
                if (JSONValue !== undefined) {
                    ...
                    //self.degree.set('difficult_degree', modelVal);
                    //console.log("model create : value ["+modelVal.value + "]");
                }
            });
    },
    saveView : function() {
        ...
        //    check for dupes
        var self = this;
        //console.log("model difficult_degree value : ",this.model.get('difficult_degree'));
        //console.log("oldValue : "+this.model.get('difficult_degree'));
        var x = this.model.get("difficult_degree");
        //console.log( x['display_name'], x['value'], x['help']);

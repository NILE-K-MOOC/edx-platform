    ...
    def create(self, request):
        ...
        if(course_module != None):
            ...
            if (has_course_module != None):
                ...
                rerun_course(unicode(src_course_key), unicode(des_course_key), user_data.id, json_fields)
                #rerun_course.delay(unicode(src_course_key), unicode(des_course_key), user_data.id, json_fields)
                ### Course COPY OK ###
                log.info("####### RERUN END #######")
                return Response({"result": "ok"})
            else:
